...
function loadView(){
	...
	if(currentNode){
		...
		$.ajax({
			type : "post",
			url : encodeURI(pageContext + "/event/toListEventPage?regionID="+currentNode.id+"&timestamp="+Date.parse(new Date())),
			dataType : "html",
			data : null,
		...
		$.ajax({
			type : "post",
			url : encodeURI(pageContext + "/event/toListEventPage?regionID="+$("#firstRegionID").val()+"&timeMethod="+$("#timeMethod").val()+"&timestamp="+Date.parse(new Date())),
			dataType : "html",
			data : null,
			success : function(data) {
				//alert(data);
...
function showfenxi(obj){
	...
	$("#viewDiv").html('<div class="x_jiazai"><div class="tuxing_dengdai"></div><p>正在加载</p></div>');
	$("#viewDiv").load(pageContext + "/event/showfenxi?regionID="+regionID+"&eventid="+eventid+"&timestamp="+Date.parse(new Date()), function() {
		afterLoad();
	});
}
...
function to_ListEvent(eventid,obj){
	...
	var conditions ="&eventid="+eventid+"&polarityorientation=-2&timeMethod=2&pagesize=10&type=0&pageNo=1";
	var regionID = $("#firstRegionID").val();
	if(currentNode!=undefined){
		regionID = currentNode.id;
	}
	$("#viewDiv").html('<div class="x_jiazai"><div class="tuxing_dengdai"></div><p>正在加载</p></div>');
	$("#viewDiv").load(pageContext + "/event/toListEventPage?regionID="+regionID+conditions+"&timestamp="+Date.parse(new Date()), function() {
		afterLoad();
	});
...
function addAllToggle() {
	var allTitles = generateTitles();

	allTitles.forEach(function(title) {
		if (selectOtherByTitle(title).length >= 1) {
			addToggleByTitle(title);
		}
	})

}

function addToggleByTitle(title) {
	var toggleButton = $('<a/>', {
	...
	selectOtherByTitle(title).hide();

	toggleButton.toggle(function() {
		$(this).text('[-]');
		selectOtherByTitle(title).show();
	}, function() {
		$(this).text('[+]');
		selectOtherByTitle(title).hide();
	});

	selectFirstByTitle(title).find('a').before(toggleButton);
}
...
function setEventName(){
	$('.eventProperty').each(function() {
		var _this = $( this );
		var id = _this.attr('eventid');
		var name = eventDict[id];
		_this.html('['+name+']');
	});
}

/**
 * 生成eventid与eventname对应的dict
 * @param id
 * @return
 */
function generateEventDict(){
	var event_dict = {};
	$('.baikuang ul li').each(function() {
		var _this = $( this );
		var event_id = _this.attr('onclick').match(/-?\d+/)[0];
		var event_name = _this.html();
...
function generateTitles(){
	var titles = $('h1.color_hongse_n a');
	var titleSet = new Set();

	titles.each(function() {
		titleSet.add($(this).attr('title'));
	});

	return Array.from(titleSet);
}

function selectFirstByTitle(title) {
	var titles = $('h1.color_hongse_n a');
...
function selectByTitle(_title) {
	return $("[title='"+_title+"']").parent().parent().parent();
}

function selectFirstByTitle(title) {
	return selectByTitle(title).first();
}

function selectOtherByTitle(title) {
	return selectByTitle(title).not(selectFirstByTitle(title));
}
...
function fenximySubmit(obj){
	...
	$("#fenxileibiao").show();
	$("#yuqing_list").show();
	var conditions ="&eventid="+eventid+"&polarityorientation="+$("#polarityorientation").val()+"&timeMethod="+$("#timeMethod").val()+"&pagesize="+$("#pagesize").val()+"&type="+$("#type").val()+"&pageNo=1";
	if(currentNode!=undefined){
		regionID = currentNode.id;
	}
	$("#viewDiv").load(pageContext + "/event/toListEventPage?regionID="+regionID+conditions+"&timestamp="+Date.parse(new Date()), function() {
	...
	$.ajax({
		...
		success : function(data) {
			if(data<3){
				...
				jBox("iframe:" + pageContext + "/event/toAddTopic/?currentEventnewsId="+pageid+"&regionID="+$("#regionID").val(), {
					title : "添加专题",
					width : 500,
					id : "addTopic",
					height : 550,
...
function setAttentionId(attentionId,attentionName,pageId){
	...
	var submit = function (v, h, f) {
		if (v == 'ok') {
			...
			$.ajax({
				type : "post",
				url : encodeURI(pageContext + "/attention/addToAttentionByEvennews"),
				dataType : "json",
				data : params,
				success : function(data) {
					if(data>0){
...
function setBriefreportId(briefreportId,briefreportName,pageId){
	...
	var submit = function (v, h, f) {
		if (v == 'ok') {
			...
			$.ajax({
				type : "post",
				url : encodeURI(pageContext + "/briefreportInfo/saveToBriefreportByEvennews"),
				dataType : "json",
				data : params,
				success : function(data) {
					if(data>0){
...
function loadEventnews(eventnewsId){
	window.open(pageContext+"/event/loadEventnews?currentEventnewsId="+eventnewsId+"&regionID="+$("#regionID").val());
}
/**
 * 导出
 */
function exportEvent(){
	...
	if(val=='1'||val=='3'){
		...
		if(ids.length==0){
			...
			if(val=='1'){
				parent.location.href=pageContext+"/event/downloadByIds/"+ids;
			}else{
				parent.location.href=pageContext+"/event/downloadXmlByIds/"+ids;
			}
