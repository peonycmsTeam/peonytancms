...
function loadView() {
    if (currentNode) {
        initSel();
        //$("#viewDiv").load(pageContext + "/subject/toListSubjectSentimentOld?subjectid="+currentNode.id+"&timestamp="+Date.parse(new Date()));
        $("#viewDiv").html('<div class="x_jiazai"><div class="tuxing_dengdai"></div><p>正在加载</p></div>');
        $.ajax({
            type: "post",
            url: encodeURI(pageContext + "/subject/toListSubjectSentimentOld?subjectid=" + currentNode.id + "&timestamp=" + Date.parse(new Date())),
            dataType: "html",
            data: null,
        ...
        //$("#viewDiv").load(pageContext + "/subject/toListSubjectSentimentOld?subjectid=1"+$("#homePage").val()+"&timestamp="+Date.parse(new Date()));
        $.ajax({
            type: "post",
            url: encodeURI(pageContext + "/subject/toListSubjectSentimentOld?subjectid=1" + $("#homePage").val() + "&time=" + $("#time").val() + "&timestamp=" + Date.parse(new Date())),
            dataType: "html",
            data: null,
            success: function (data) {
                //alert(data);
...
function showfenxi(obj) {
    ...
    $("#viewDiv").html('');
    $("#viewDiv").load(pageContext + "/subject/toshowfenxi?subjectid=" + subjectid + "&timestamp=" + Date.parse(new Date()));
}
...
function fenxisubmit(obj) {
    ...
    $("#yuqing_top").show();
    $("#yuqing_list").show();
    $("#viewDiv").load(pageContext + "/subject/toListSubjectSentimentOld?subjectid=" + subjectid + "&timestamp=" + Date.parse(new Date()));
}
/**
 * 通过左侧导航查询信息
 * @param id
...
function tofenxi_subject_sentiment(id) {
    $("#pageNo").val(1);
    $("#title").val("");
    $("#type").val(0);
    $("#polarity").val(-2);
    ...
    $.ajax({
        type: "post",
        url: encodeURI(pageContext + "/subject/toListSubjectSentimentOld?subjectid=" + subjectId + conditions + "&timestamp=" + Date.parse(new Date())),
        dataType: "html",
        data: null,
        success: function (data) {
            //alert(data);
                ...
                success: function (data) {
                    if (data > 0) {
                        ...
                        window.setTimeout(function () {
                            $.jBox.tip("信息删除成功", 'success', 1000);
                        }, 100);
                        window.setTimeout(mySubmit, 1000);
                    } else {
                        window.setTimeout(function () {
                            $.jBox.tip("信息删除失败", 'error', 1000);
                        }, 100);
                    }
                }
            });
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data < 3) {
                ...
                jBox("iframe:" + pageContext + "/subject/toAddTopic?currentId=" + pageid, {
...
function loadSubjectPage(id, groupseedid) {
    ...
    window.open(pageContext + "/subject/loadSubjectPage?currentId=" + id + "&groupseedid=" + groupseedid);
...
//				parent.location.href=pageContext+"/subject/downloadExcelByIds/"+ids;
                jBox("iframe:" + pageContext + "/subject/toDownloadExcelByIds/" + ids, {
                    title: "选择导出方式",
                    width: 400,
                    height: 155,
                    top: '15%',
    ...
    if (val == '2' || val == '4') {
        jBox("iframe:" + pageContext + "/subject/toExportByTime/" + val, {
            title: "选择导出时间",
            width: 400,
            height: 215,
            top: '15%',
...
function selectWay() {
    parent.showPop();
    var ids = $("#ids").val();
    var isColspan = $("input[name='isColspan']:checked").val();
    var form = parent.document.forms[0];
    form.action = pageContext + "/subject/downloadExcelByIds?ids=" + ids + "&isColspan=" + isColspan;
    form.method = "post";
    form.submit();
//	parent.location.href = pageContext+"/subject/downloadExcelByIds?ids="+ids+"&isColspan="+isColspan;
}
/**
 * 按选择时间导出
 */
    ...
    if (btime == '') {
        ...
        if (etime == '') {
            ...
            if (end < start) {
                ...
                var polarity = $("#polarity", window.parent.document).val();
                var isRead = $("#isRead", window.parent.document).val();
                var isColspan = $("input[name='isColspan']:checked").val();
                var polarityorientation = $("#polarityorientation", window.parent.document).val();
                var form = parent.document.forms[0];
                if (val == '2') {
                    form.action = pageContext + "/subject/downloadBySelectTime?title=" + title + "&type=" + type + "&polarity=" + polarity + "&isRead" + isRead + "&polarityorientation=" + polarityorientation;
                } else {
                    form.action = pageContext + "/subject/downloadExcelBySelectTime?title=" + title + "&type=" + type + "&polarity=" + polarity + "&isRead" + isRead + "&polarityorientation=" + polarityorientation + "&isColspan=" + isColspan;
                }
                form.method = "post";
                form.submit();
//			    	alert(form.action);
