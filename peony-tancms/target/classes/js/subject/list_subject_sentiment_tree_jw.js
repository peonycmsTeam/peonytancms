...
function loadView() {
    if (currentNode) {
        ...
        $.ajax({
            type : "post",
            url : encodeURI(pageContext + "/subject/toListOverseasSubjectSentimentOld?subjectid=" + currentNode.id + "&timestamp=" + Date.parse(new Date())),
            dataType : "html",
            data : null,
        ...
        // "/subject/toListOverseasSubjectSentimentOld?subjectid=1&timestamp="+Date.parse(new
        // Date()));
        $.ajax({
            type : "post",
            url : encodeURI(pageContext + "/subject/toListOverseasSubjectSentimentOld?subjectid=1&timestamp=" + Date.parse(new Date())),
            dataType : "html",
            data : null,
            success : function(data) {
                document.getElementById("viewDiv").innerHTML = data;
...
function showfenxi(obj) {
    ...
    $("#viewDiv").html('');
    $("#viewDiv").load(pageContext + "/subject/toshowJYfenxi?subjectid=" + subjectid + "&timestamp=" + Date.parse(new Date()));
}
...
function fenxisubmit(obj) {
    ...
    $("#yuqing_top").show();
    $("#yuqing_list").show();
    $("#viewDiv").load(pageContext + "/subject/toListOverseasSubjectSentimentOld?subjectid=" + subjectid + "&timestamp=" + Date.parse(new Date()));
}
/**
 * 通过左侧导航查询信息
 *
...
function tofenxi_subject_sentiment(id) {
    ...
    $.ajax({
        type : "post",
        url : encodeURI(pageContext + "/subject/toListOverseasSubjectSentimentOld?subjectid=" + subjectId + conditions + "&timestamp=" + Date.parse(new Date())),
        dataType : "html",
        data : null,
        success : function(data) {
            document.getElementById("viewDiv").innerHTML = data;
...
function mySubmit() {
    ...
    var conditions = "&title=" + $("#title").val() + "&type=" + $("#type").val() + "&polarity=" + $("#polarity").val() + "&isRead=" + $("#isRead").val() + "&time=" + $("#time").val() + "&pageNo=" + pageNo + "&pagesize=" + $("#pagesize").val();
    var subjectId = 1;
    if (currentNode != undefined) {
        subjectId = currentNode.id;
    }
    $("#viewDiv").html('<div class="x_jiazai"><div class="tuxing_dengdai"></div><p>正在加载</p></div>');
    // $("#viewDiv").load(pageContext +
    // "/subject/toListOverseasSubjectSentimentOld?subjectid="+subjectId+conditions+"&timestamp="+Date.parse(new
    // Date()));
    $.ajax({
        type : "post",
        url : encodeURI(pageContext + "/subject/toListOverseasSubjectSentimentOld?subjectid=" + subjectId + conditions + "&timestamp=" + Date.parse(new Date())),
    ...
    if (btime == '') {
        ...
        if (etime == '') {
            ...
            if (end < start) {
                ...
                form.action = pageContext + "/subject/downloadBySelectTime";
                form.method = "post";
                form.submit();
            }
        }
