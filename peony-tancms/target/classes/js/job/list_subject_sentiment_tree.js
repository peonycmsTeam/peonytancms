...
function loadView() {
	...
	// "/subject/toListSubjectSentimentOld?subjectid="+currentNode.id+"&timestamp="+Date.parse(new
	// Date()));
	$("#viewDiv")
			.html(
	...
	$.ajax({
		...
		url : encodeURI(pageContext
				+ "/job/toListSubjectSentimentOld?subjectid=5032&timestamp="
				+ Date.parse(new Date())),
		dataType : "html",
		data : null,
...
function showfenxi(obj) {
	...
	$("#viewDiv").html('');
	$("#viewDiv").load(
			pageContext + "/subject/toshowfenxi?subjectid=" + subjectid
...
function fenxisubmit(obj) {
	...
	$("#yuqing_list").show();
	$("#viewDiv").load(
			pageContext + "/subject/toListSubjectSentimentOld?subjectid="
					+ subjectid + "&timestamp=" + Date.parse(new Date()));
}
/**
 * 通过左侧导航查询信息
 * 
...
function tofenxi_subject_sentiment(id) {
	...
	$("#time").val(2);
	$("#subjectid").val(id);
	var conditions = "&title=" + $("#title").val() + "&type="
			+ $("#type").val() + "&polarity=" + $("#polarity").val()
			+ "&isRead=" + $("#isRead").val() + "&time=" + $("#time").val()
			+ "&pageNo=" + $("#pageNo").val() + "&pagesize="
			+ $("#pagesize").val();
	var subjectId = 1;
	...
	$("#viewDiv")
			.html(
					'<div class="x_jiazai"><div class="tuxing_dengdai"></div><p>正在加载</p></div>');
	// $("#viewDiv").load(pageContext +
	// "/subject/toListSubjectSentimentOld?subjectid="+subjectId+conditions+"&timestamp="+Date.parse(new
	// Date()));
	$.ajax({
		type : "post",
		url : encodeURI(pageContext
...
function mySubmit() {
	...
	$.ajax({
		type : "post",
		url : encodeURI(pageContext
				+ "/job/toListSubjectSentimentOld?subjectid=5032"+conditions+"&timestamp="
				+ Date.parse(new Date())),
		dataType : "html",
		data : null,
		success : function(data) {
	...
	var submit = function(v, h, f) {
		if (v == 'ok') {
			$.ajax({
				type : "post",
				url : encodeURI(pageContext
						+ "/subject/updateSubjectPagePolarity"),
				dataType : "json",
				data : params,
				success : function(data) {
					if (data > 0) {
...
function addWarning(pageid) {
	$
			.ajax({
				type : "post",
				url : encodeURI(pageContext + "/warning/saveWarningByPageId/"
						+ pageid),
				dataType : "json",
				data : null,
				success : function(data) {
	...
	$.ajax({
		...
		success : function(data) {
			if (data < 3) {
				...
				jBox("iframe:" + pageContext + "/subject/toAddTopic?currentId="
						+ pageid, {
	...
	var submit = function(v, h, f) {
		if (v == 'ok') {
			...
			$.ajax({
				type : "post",
				url : encodeURI(pageContext
						+ "/briefreportInfo/addToBriefreport"),
				dataType : "json",
				data : params,
				success : function(data) {
					if (data > 0) {
...
function loadSubjectPage(id, groupseedid) {
	...
	window.open(pageContext + "/subject/loadSubjectPage?currentId=" + id
...
function exportSubject() {
	...
	if (val == '1' || val == '3') {
		...
		if (ids.length == 0) {
			...
			if (val == '1') {
				parent.location.href = pageContext + "/subject/downloadByIds/"
						+ ids;
			} else {
				// parent.location.href=pageContext+"/subject/downloadExcelByIds/"+ids;
				jBox("iframe:" + pageContext + "/subject/toDownloadExcelByIds/"
						+ ids, {
					title : "选择导出方式",
					width : 400,
					height : 155,
...
function selectWay() {
	...
	form.action = pageContext + "/subject/downloadExcelByIds?ids=" + ids
			+ "&isColspan=" + isColspan;
	form.method = "post";
	form.submit();
	// parent.location.href =
	// pageContext+"/subject/downloadExcelByIds?ids="+ids+"&isColspan="+isColspan;
}
/**
 * 按选择时间导出
 */
	...
	if (btime == '') {
		...
		if (etime == '') {
			...
			if (end < start) {
				...
				var isColspan = $("input[name='isColspan']:checked").val();
				var polarityorientation = $("#polarityorientation",
						window.parent.document).val();
				var form = parent.document.forms[0];
				if (val == '2') {
					form.action = pageContext
							+ "/subject/downloadBySelectTime?title=" + title
							+ "&type=" + type + "&polarity=" + polarity
							+ "&isRead" + isRead + "&polarityorientation="
							+ polarityorientation;
				} else {
					form.action = pageContext
							+ "/subject/downloadExcelBySelectTime?title="
							+ title + "&type=" + type + "&polarity=" + polarity
							+ "&isRead" + isRead + "&polarityorientation="
							+ polarityorientation + "&isColspan=" + isColspan;
				}
				form.method = "post";
				form.submit();
				// alert(form.action);
