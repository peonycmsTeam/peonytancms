...
<form id="reprotConfigForm">
	<input type="hidden" name="briefreportConfigId" value="${briefReportConfig.briefreportConfigId}"/>
		<div class="bg">
		  <div class="zhanwei color_hongse_n"> <strong>当前位置：</strong>&nbsp;首页&nbsp;&gt;&nbsp;系统配置&nbsp;&gt;&nbsp;简报模板定制</div>
		</div>
    	<div class="bg">
	     ...
	     <div class="fl_right_in">
          ...
          <div class="baikuang">
            ...
            <div class="moban_list">
	            <ul>
	            <#if userfront.userType==2><!-- 政府用户-->
	            <#if ( briefReportConfig.briefreportTempId>0 )>
	            <#list listBriefReportTemp as reportTemp>
	                <li>
	                 <a title="预览" href="#" onClick="view('${reportTemp.name}')"> 
		              	<img src="${request.getContextPath()}/${reportTemp.url}" width="156" height="156" />
		             </a>
			              <label for="radio">
			              	<input type="radio" name="briefreportTempId" <#if briefReportConfig.briefreportTempId?string?contains("${reportTemp.briefreportTempId}")>checked</#if>  value="${reportTemp.briefreportTempId}" />
			              </label>
	              	${reportTemp.name}
	              </li>
	              </#list>
	              <#else>
	               <#list listBriefReportTemp as reportTemp>
	                ...
	                <a title="预览" href="#" onClick="view('${reportTemp.name}')"> 
			              ...
			              <label for="radio">
			              	<input type="radio" name="briefreportTempId" <#if reportTemp_index==0 >checked</#if>  value="${reportTemp.briefreportTempId}" />
			              </label>
	              	${reportTemp.name}
	              </li>
	              </#list>
...
<!----底部-开始--->
<div class="HackBox"></div>
<script type="text/javascript" src="${request.getContextPath()}/js/briefreport/briefreport_config.js"></script>
